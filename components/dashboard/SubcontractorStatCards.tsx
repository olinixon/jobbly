...
export default function SubcontractorStatCards({ initialStats, campaignId, dateRange, from, to }: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setStats({
        ...
        totalJobsRevenue: data.totalJobsRevenue,
      })
    } finally {
  ...
  const cards = [
    ...
    { label: 'Total Jobs Revenue (ex GST)', value: `$${stats.totalJobsRevenue.toFixed(2)}` },
  ]
