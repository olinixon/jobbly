...
function FileDropZone({ onFile, fileInputRef }: { onFile: (f: File) => void; fileInputRef: React.RefObject<HTMLInputElement | null> }) {
  ...
  return (
    <>
      <div
        ...
        onDrop={handleDrop}
        className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-all cursor-pointer ${dragOver ? 'border-[#2563EB] bg-blue-50 dark:bg-blue-950/30' : 'border-[#D1D5DB] dark:border-[#334155] bg-[#F9FAFB] dark:bg-[#0F172A]'}`}
      ...
      {fileError && <p className="text-sm text-[#DC2626] mt-1">{fileError}</p>}
    </>
  )
}
...
export default function JobActions({
  ...
  hasInvoice,
  invoiceUrl,
  hasJobReport: initialHasJobReport,
  jobReportUrl: initialJobReportUrl,
  jobReportFileName: initialJobReportFileName,
  markupPercentage,
  jobTypes: _jobTypes,
  ...
  const bothFilesReady = hasInvoice && hasJobReport
  ...
  if (currentStatus === 'JOB_COMPLETED') {
    // Read-only documents view after completion
    return (
  ...
  if (currentStatus === 'JOB_BOOKED') {
    return (
      <>
        {/* Success banner */}
        {submitSuccess && (
          <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-green-600 text-white px-6 py-3 rounded-xl shadow-lg text-sm font-medium flex items-center gap-2">
        ...
        <div className="bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] rounded-xl p-6 shadow-sm space-y-5">
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium text-[#374151] dark:text-[#CBD5E1]">Invoice</p>
            {hasInvoice && invoiceUrl ? (
              <div className="flex items-center gap-3">
                <span className="text-sm text-[#111827] dark:text-[#F1F5F9] flex items-center gap-1">
                ...
                <a href={invoiceUrl} download className="text-xs text-[#6B7280] dark:text-[#94A3B8] hover:text-[#2563EB] hover:underline">Download</a>
                <button
                  onClick={() => { closeInvoiceModal(); setShowInvoiceModal(true) }}
                  className="text-xs text-[#9CA3AF] dark:text-[#475569] hover:text-[#6B7280] dark:hover:text-[#94A3B8] underline"
                >
                  Replace
                </button>
              </div>
          ...
          {/* Job report section */}
          <div className="space-y-2">
            <p className="text-sm font-medium text-[#374151] dark:text-[#CBD5E1]">Job Report</p>
            {hasJobReport && jobReportUrl ? (
              <div className="flex items-center gap-3">
                <span className="text-sm text-[#111827] dark:text-[#F1F5F9] flex items-center gap-1">
                  <svg className="w-4 h-4 text-green-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                  {jobReportFileName ?? 'Report attached'}
                </span>
                <a href={jobReportUrl} download className="text-xs text-[#6B7280] dark:text-[#94A3B8] hover:text-[#2563EB] hover:underline">Download</a>
                <button
                  onClick={() => { setJobReportError(''); setShowJobReportModal(true) }}
                  className="text-xs text-[#9CA3AF] dark:text-[#475569] hover:text-[#6B7280] dark:hover:text-[#94A3B8] underline"
                >
                  Replace
                </button>
              ...
              <Button onClick={() => { setJobReportError(''); setShowJobReportModal(true) }}>
                Attach Job Report
              </Button>
            )}
          </div>
          ...
          <div className="pt-2 border-t border-[#F3F4F6] dark:border-[#334155]">
            ...
            <div className="group relative inline-block w-full">
              <Button
                onClick={submitJob}
                disabled={!bothFilesReady || submitting || submitSuccess}
                className="w-full"
              >
                {submitting ? 'Submitting…' : submitSuccess ? 'Submitted ✓' : 'Submit Job'}
              </Button>
              {!bothFilesReady && (
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block bg-[#111827] text-white text-xs rounded-lg px-3 py-1.5 whitespace-nowrap pointer-events-none z-10">
                  Upload both the invoice and job report to submit
                </div>
              )}
            </div>
          </div>
          ...
          <Modal title="Attach Invoice" onClose={closeInvoiceModal}>
            {uploadStep === 'drop' && (
              <div className="space-y-4">
                ...
                >
                  <input ref={invoiceFileInputRef} type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" onChange={handleFileChange} />
                  {selectedFile ? (
                    <div className="flex flex-col items-center gap-2">
        ...
        {showJobReportModal && (
          <Modal title="Attach Job Report" onClose={() => { setShowJobReportModal(false); setJobReportUploadFile(null); setJobReportError('') }}>
            <div className="space-y-4">
              <FileDropZone
                onFile={(f) => setJobReportUploadFile(f)}
                fileInputRef={jobReportFileInputRef}
              />
              ...
              <div className="flex gap-3 justify-end">
                ...
                <Button onClick={uploadJobReport} disabled={!jobReportUploadFile || jobReportUploading}>
                  {jobReportUploading ? 'Uploading…' : 'Upload'}
                </Button>
              </div>
            </div>
          </Modal>
  ...
  // LEAD_RECEIVED or QUOTE_SENT — current status card only (no invoice button shown here)
  return (
    <>
