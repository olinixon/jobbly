...
export default function LeadActions({
  quoteNumber, currentStatus, hasInvoice, hasQuote, markupPercentage: _markupPercentage, customerName: _customerName, propertyAddress: _propertyAddress, role,
}: LeadActionsProps) {
  ...
  const [quoteMismatch, setQuoteMismatch] = useState<{
  ...
  // Job booked date — used when advancing legacy QUOTE_SENT → JOB_BOOKED
  ...
  // Advance legacy QUOTE_SENT lead to JOB_BOOKED
  async function advanceToJobBooked() {
    ...
    const res = await fetch(`/api/leads/${quoteNumber}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'JOB_BOOKED', jobBookedDate: d.toISOString() }),
    })
    setSaving(false)
    if (!res.ok) { const d2 = await res.json(); setError(d2.error ?? 'Something went wrong.'); return }
  ...
  function handleQuoteFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    const f = e.target.files?.[0]
    if (!f) return
    const err = validateFile(f)
    ...
    <>
      <div className="bg-white dark:bg-[#1E293B] border border-[#E5E7EB] dark:border-[#334155] rounded-xl p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-3">
          {/* Legacy QUOTE_SENT lead — admin can manually advance to JOB_BOOKED */}
          ...
          {/* Upload Quote — Admin and Subcontractor only, all statuses */}
          {role !== 'CLIENT' && (
      ...
      {/* Status modal — legacy QUOTE_SENT → JOB_BOOKED */}
      {showStatusModal && (
        <Modal title="Move to Job Booked" onClose={() => setShowStatusModal(false)}>
          <p className="text-sm text-[#6B7280] dark:text-[#94A3B8] mb-4">
            Move this lead to <strong>{STATUS_LABELS['JOB_BOOKED']}</strong>? Enter the booked job date below.
          </p>
          <div className="mb-4">
          ...
          <div className="flex gap-3 justify-end">
            ...
            <Button onClick={advanceToJobBooked} disabled={saving || !bookedDateFilled || !bookedDateValid}>
              {saving ? 'Saving…' : 'Confirm — Move to Job Booked'}
            </Button>
          </div>
        </Modal>
      )}
