...
export async function POST(request: NextRequest) {
  ...
  const { stripe_secret_key, stripe_gst_rate_id, stripe_customer_id, company_name, billing_email, billing_address, payment_only } = body
  const isPaymentOnly = payment_only === true
  ...
  // Step A — verify the secret key (and customer ID for full invoicing flow)
  ...
  // Encrypt key and upsert BillingProfile
  ...
  await prisma.billingProfile.upsert({
    ...
    create: {
      ...
      stripe_customer_id: isPaymentOnly ? null : stripe_customer_id,
      stripe_secret_key: encryptedKey,
      stripe_gst_rate_id: isPaymentOnly ? null : stripe_gst_rate_id,
